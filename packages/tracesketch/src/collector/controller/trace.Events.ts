...
if (!eventType || !service || !operation || durationMs === undefined || durationMs === null || durationMs === "") {
    return res.status(400).json({
        message: "Missing required fields"
    });
}
const result = await insertTraceEvent(traceId, eventType, service, operation, Number(durationMs), metadata ?? "", Date.now());
if (!result) {
